...
from cortex.agent_marl import AdaptiveCortexAgent  # noqa: E402
from cortex.constitution_client import ConstitutionIPCClient  # noqa: E402
from simulation.synthetic_gen import SyntheticRegimeGenerator  # noqa: E402

CONSTITUTIOND = os.path.abspath(
    os.path.join(os.path.dirname(__file__), "../../constitution/target/debug/constitutiond.exe")
)
...
def run_simulation(years: int = 250) -> dict:
    ...
    with ConstitutionIPCClient() as client:
        for year in range(years):
            ...
            # End-of-year: if halted, attempt cryptographic recovery so the
            # system can re-enter Normal operations with the restructured capital.
...
if __name__ == "__main__":
    report = run_simulation(years=250)
